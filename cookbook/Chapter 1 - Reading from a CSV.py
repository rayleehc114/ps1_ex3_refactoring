...
# TODO: how would you do this with a Polars data frame?
# 方法1: 转换为pandas然后绘图 (需要pyarrow)
# pl_fixed_df.to_pandas()["Berri 1"].plot()

# 方法2: 使用matplotlib直接绘图 (推荐)

plt.figure(figsize=(10, 6))
plt.plot(pl_fixed_df["Berri 1"].to_numpy())
plt.title("Berri 1")
plt.xlabel("Index")
plt.ylabel("Value")
plt.show()
...
for col in pl_fixed_df.columns:
    if col != "Date":  # 跳过日期列
        plt.plot(pl_fixed_df[col].to_numpy(), label=col)
plt.title("All Bike Paths")
plt.xlabel("Index")
plt.ylabel("Value")
plt.legend()
plt.show()
